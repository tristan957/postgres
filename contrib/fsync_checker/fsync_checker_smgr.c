...
static void fsync_checker_write(SMgrRelation reln, ForkNumber forknum,
...
static const struct f_smgr fsync_checker_smgr = {
	...
	.smgr_read = mdread,
	.smgr_write = fsync_checker_write,
...
_PG_init(void)
{
	...
	/*
	 * Relation size of 0 means we can just defer to md, but it would be nice to
	 * just expose this functionality, so if I needed my own relation, I could
	 * use MdSmgrRelation as the parent.
	 */
	fsync_checker_smgr_id = smgr_register(&fsync_checker_smgr, 0);

	storage_manager_id = fsync_checker_smgr_id;
	storage_manager_string = "fsync_checker";
}
...
fsync_checker_checkpoint_create(const CheckPoint *checkPoint)
{
	...
	while ((entry = hash_seq_search(&status))) {
		if (entry->lsn < checkPoint->redo) {
			...
			elog(WARNING, "File not previously synced: %s", path);
...
static void
fsync_checker_write(SMgrRelation reln, ForkNumber forknum,
					BlockNumber blocknum, const void *buffer, bool skipFsync)
{
	if (!SmgrIsTemp(reln) && skipFsync)
		add_reln(reln, forknum);
