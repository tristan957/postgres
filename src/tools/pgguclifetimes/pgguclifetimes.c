...
#define POSTMASTER_GUC "postmaster_guc"
#define SESSION_GUC "session_guc"
#define SIGHUP_GUC "sighup_guc"
...
static const char *guc_lifetimes[] = { POSTMASTER_GUC, SESSION_GUC, SIGHUP_GUC };
...
visit_AnnotateAttr(CXCursor cursor, CXCursor parent, CXClientData client_data)
{
	bool *annotated;
...
static enum CXChildVisitResult
visit_VarDecl(CXCursor cursor, CXCursor parent, CXClientData client_data)
{
	Assert(parent.kind == CXCursor_VarDecl);

	switch (cursor.kind) {
	case CXCursor_AnnotateAttr:
		return visit_AnnotateAttr(cursor, parent, client_data);
	default:
...
visit_global_variable(CXCursor cursor, CXCursor parent, CXClientData client_data)
{
	...
	tls_kind = clang_getCursorTLSKind(cursor);
	if (tls_kind != CXTLS_Static) {
		(*issues)++;
		annotated = false;
		error(cursor, "is not marked as thread-local storage");
		if (fail_fast)
			goto out;
	}

	clang_visitChildren(cursor, visit_VarDecl, ctx);
...
visit_config_type(CXCursor cursor, CXClientData client_data)
{
	...
	field = clang_getCursorSpelling(cursor);
	if (strcmp(clang_getCString(field), "variable") == 0) {
...
main(const int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "+:1ehiq", options, &longind)) != -1) {
		...
		case 'e':
			...
			if (!vector_append(&excludes, optarg)) {
				rc = 1;
				fputs("out of memory\n", stderr);
				goto out;
			}
		...
		case 'i':
			...
			if (!vector_append(&includes, optarg)) {
				rc = 1;
				fputs("out of memory\n", stderr);
				goto out;
			}
